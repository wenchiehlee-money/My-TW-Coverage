...
- Theme pages render a single `## 相關公司` list — there is no `上游`/`中游`/`下游` chapter split. The upstream TPEx position tagging (`raw_SupplyChain_*.csv` `位置` column) was confirmed unreliable enough (see `skill-theme-competitor-groups-curate/references/known_gaps.md`) that splitting real competitor groups across position-based chapters caused more confusion than it resolved; a positional split was tried and deliberately removed.
- Within that single list, companies group by `data/themes/*.json` `competitive_groups` first (curated real product/business-model competitor groups, rendered in the JSON's declared order), then fall back to IC-taxonomy subcategory or GICS `profile.sector`/`profile.industry` for ungrouped companies (sorted by descending total group market cap). See `skill-theme-competitor-groups-curate/SKILL.md` for how to curate `competitive_groups`.
...
- `data/themes/*.json` may also define `extra_entities` to inject a company the TPEx source omits entirely from every matching row (a genuine source-data gap, not a shortcut around checking taxonomy criteria — see `skill-theme-competitor-groups-curate/SKILL.md`).
