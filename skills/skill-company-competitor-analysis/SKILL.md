...
Populate `Event` from `../biztrends.TW/data/InvestorEvents/raw_event_upcoming_earnings.csv` when a matching `(stock, period)` `財報公告` or `法說會` row exists. Leave it blank when the event source has no matching row.
