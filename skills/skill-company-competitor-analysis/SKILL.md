...
For monthly-revenue-only quarters, show a matching `財報公告` or `法說會` date from `../biztrends.TW/data/InvestorEvents/raw_event_upcoming_earnings.csv` in the first otherwise-empty financial cell, normally `Profit`. Leave cells blank when the event source has no matching row.
